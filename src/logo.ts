...
import { mascotLines } from "./mascot-data";

function renderLineToSVG(charsAndColors: { char: string, color: string | null, bg: string | null }[], charWidth: number): string {
...
export function printLogo() {
    ...
    for (const line of mascotLines) {
        ...
        for (const item of line) {
            ...
            if (item.fg && item.bg) {
                ...
                if (fgMatches && bgMatches) {
                    ...
                    consoleLine += chalk.rgb(rF, gF, bF).bgRgb(rB, gB, bB)(char);
                } else {
                    consoleLine += char;
                }
            } else if (item.fg) {
                ...
                if (fgMatches) {
                    ...
                    const gF = parseInt(fgMatches[2], 16);
                    const bF = parseInt(fgMatches[3], 16);
                    consoleLine += chalk.rgb(rF, gF, bF)(char);
                } else {
                    consoleLine += char;
                }
            } else if (item.bg) {
                ...
                if (bgMatches) {
                    ...
                    const gB = parseInt(bgMatches[2], 16);
                    const bB = parseInt(bgMatches[3], 16);
                    consoleLine += chalk.bgRgb(rB, gB, bB)(char);
                } else {
                    consoleLine += char;
                }
            } else {
        ...
        if (consoleLine.trim().length === 0) continue;
...
export function generateSVG() {
    ...
    for (let y = 0; y < mascotLines.length; y++) {
        const line = mascotLines[y];
    ...
    for (let y = 0; y < titleLines.length; y++) {
        ...
        const charsAndColors: { char: string, color: string | null, bg: string | null }[] = [];
